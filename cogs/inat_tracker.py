...
class InatTrackerCog(commands.Cog, name="InatTracker"):
    ...
    async def on_message(self, message: discord.Message):
        ...
        for obs_id in obs_ids:
            ...
            if self._link_exists(ws, url):
                logger.info(
                    f"⏭️  iNat obs {obs_id} bereits im Sheet – übersprungen "
                    f"(user={discord_id}, {display_name})"
                )
                continue
            ...
            if is_ant is None:
                ...
                asyncio.create_task(self._retry_pending(message, obs_id, ws, discord_id, display_name))
                continue
            if not is_ant:
                logger.info(
                    f"⏭️  iNat obs {obs_id} gehört nicht zu Formicoidea "
    ...
    async def _retry_pending(
        ...
        while True:
            ...
            is_ant = await self._is_formicoidea(obs_id)
            if is_ant is None:
                logger.warning(f"⚠️  iNat API noch nicht erreichbar (obs {obs_id}, Versuch {attempt})")
                continue  # nochmal warten
            # API hat geantwortet – ⏳ entfernen
            try:
            ...
            if not is_ant:
                logger.info(
                    f"⏭️  iNat obs {obs_id} gehört nicht zu Formicoidea – übersprungen"
                )
                return
            # Nochmals prüfen ob der Link zwischenzeitlich eingetragen wurde
            if self._link_exists(ws, url):
                logger.info(f"⏭️  iNat obs {obs_id} inzwischen bereits im Sheet")
                return
            try:
                row = self._write_entry(ws, discord_id, display_name, url)
